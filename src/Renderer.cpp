...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        VkDescriptorBufferInfo grassBladesBufferInfo = {};
        grassBladesBufferInfo.buffer = scene->GetBlades()[i]->GetBladesBuffer();
        grassBladesBufferInfo.offset = 0;
        grassBladesBufferInfo.range = NUM_BLADES * sizeof(Blade);
        ...
        numBladesBufferInfo.range = NUM_BLADES * sizeof(Blade);
        ...
        descriptorWrites[3 * i + 0].pBufferInfo = &grassBladesBufferInfo;
